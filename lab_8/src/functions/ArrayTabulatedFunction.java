    ...
    public Iterator<FunctionPoint> iterator() {
        return new Iterator<>() {
            ...
            public boolean hasNext() {
                return currentIndex + 1 != arr.length && arr[currentIndex + 1] != null;
            }

            @Override
            public FunctionPoint next() {
                if (hasNext())
                    return new FunctionPoint(arr[++currentIndex]);
                throw new NoSuchElementException("Точек больше нет");
            }

            @Override
            public void remove() {
                throw new UnsupportedOperationException("Не поддерживает операцию удаления");
            }
        };
    }
